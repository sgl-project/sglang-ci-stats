...
| `demo.py` | Trend viewer. Walks every run's `time_stats` and renders a single-file `demo.html` (Chart.js via CDN) with line charts of CI time over time (total / per-stage / per-runner). The total chart also carries one line per part of `pr-test.yml` -- its own `base-*` stages, the called `pr-test-extra.yml`, and each specialized called workflow -- which partition the total. The generated `demo.html` is gitignored. |
...
Line charts of total (split per `pr-test.yml` caller) / per-stage / per-runner CI time across all archived runs.
